...
public class LootHouse {
    ...
    public static int maxMoneyLootedR(int []houses){
        ...
        return maxMoneyLooted(n,houses);

    }

    public static int maxMoneyLooted(int n,int []houses){
        if(n<0){
            return 0;
        }
        ...
        //Loot the current house and consider the maximum loot from two houses ago
        int lootCurrent = houses[n] + maxMoneyLooted( n - 2,houses);

        //Skip the current house and consider the maximum loot from the previous house 
        int skipCurrent = maxMoneyLooted(n - 1,houses);

        return Math.max(lootCurrent, skipCurrent);
